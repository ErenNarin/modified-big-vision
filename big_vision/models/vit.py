...
class _Model(nn.Module):
  ...
  @nn.compact
  def __call__(self, image, *, train=False, region_masks=None):
    out = {}
    ...
    if region_masks is not None:
        pixel_embeddings = nn.Conv(
            ...
            name='embedding',
            dtype=self.dtype_mm,
        )(image)  # Shape: (1, 224, 224, 1152)

        # pixel_embeddings: (1, H, W, C)
        # region_masks: (1, num_regions, H, W)

        # First, reshape
        pixel_embeddings_t = jnp.transpose(pixel_embeddings, (0, 3, 1, 2))  # (1, C, H, W)

        # Pool: (1, num_regions, H, W) × (1, C, H, W) → (1, num_regions, C)
        numerator = jnp.einsum('bnxy,bcxy->bnc', region_masks, pixel_embeddings_t)  # TODO-eren: 'bnxy,bcxy->bnc' or 'bnx,bcxy->bnc'
        denominator = region_masks.sum(axis=(2, 3), keepdims=False) + 1e-6  # avoid div by 0
        pooled_embeddings = numerator / (denominator[..., None])  # shape: (1, num_regions, width)

        n, hw, c = numerator.shape
        h = w = int(hw ** (1/2))  # input image is square

        x = out["stem"] = pooled_embeddings  # Shape: (1, num_regions, width)
    else:
        x = out["stem"] = nn.Conv(
